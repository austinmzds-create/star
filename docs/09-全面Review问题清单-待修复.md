...
8. **【实测】OSS bucket 公共读写 + 匿名直传 = 全网可覆盖/删除/遍历所有文件**
...
9. **【实测】上传无类型白名单 → 存储型 XSS 可偷 token**
...
10. **【读码】寄样列表/计数全表拉进内存去重再分页,无 SQL limit**
...
11. **【读码】workbench 三级 `IN` 全量拉取,还被 30s 轮询 + 每次写操作触发**
...
1. **第一批(安全+丢数据,必做)**:P0 的 1–9。其中 8、9(OSS 私有化+上传白名单)是唯一的对外安全敞口,建议最先。
2. **第二批(前端体感)**:第四节全部 + P0-11(workbench)+ 缩略图策略(P1-存储 3 条)。这是解决"刷新 >1s"的关键。
