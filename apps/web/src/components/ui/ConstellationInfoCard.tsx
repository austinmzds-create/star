...
// ── 分区入场动画（与 StarInfoCard 同一套节奏；reduced-motion 退化纯透明度） ──

function usePanelVariants(reduceMotion: boolean): { panel: Variants; item: Variants } {
  return useMemo(() => {
    if (reduceMotion) {
      return {
        panel: {
...
function ConstellationRichPanel() {
  ...
  const { panel, item } = usePanelVariants(!!reduceMotion);
...
function ObjectRow({
  ...
  return (
    <button
      onClick={onClick}
      className="flex w-full items-baseline justify-between gap-3 rounded-xl bg-white/[0.03] px-3 py-2 text-left transition hover:bg-white/[0.08]"
    >
        ...
        >
          <div className="glass rounded-2xl p-4">
            ...
            <button
              ...
              className="mt-2.5 w-full rounded-lg bg-white/[0.06] px-3 py-1.5 text-[12px] text-nebula-100 transition hover:bg-white/[0.12] hover:text-white"
            >
              查看完整档案 →
            </button>
